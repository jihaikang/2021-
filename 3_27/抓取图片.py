...
import requests
from lxml import etree # 爬虫test最终作品
 
# qingchun/xinggan
 
page_type = ["xinggan", "qingchun", "xiaohua", "chemo", "qipao", "mingxing"]
page_max = [229, 32, 6, 10, 4, 5]
page_idx = [6, 1, 2, 3, 4, 8]
...
class Mm131Parser:
    def __init__(
        self,
        save_path="../" + page_type[type_num] + "/",
        use_parallel=True,
        save=True,
    ):
        self.base_url = "http://www.mm131.net/" + page_type[type_num] + "/"
    ...
    def _ids_titles(self, page=None):
        ...
        try:
            ...
            for index, title in enumerate(titles):
                ...
                # latest_id = latest_id.replace("un/", "")
                # print(latest_id)
    ...
    def _ids_titles_for_search(self, key, page="1"):
        ...
        try:
            ...
            page_count = str(
                content.xpath('//div[@class="dede_pages"]//a/text()')[-2]
            ).replace("...", "")
    ...
    def _one_urls(self, img_id):
        ...
        uri = (
            str(content.xpath('//div[@class="content-pic"]/a/img/@src')[0]).split(
                separator
            )[0]
            + separator
        )
    ...
    def get_all(self):
        ...
        # 以下需要根据网站变化做改动，目前总计207页(1月8日更新到229)
        for i in range(0, page_max[type_num]):
            page = "".join(["list_", str(page_idx[type_num]), "_", str(i + 2), ".html"])
            print(page)
            self.get_page(page)
...
if __name__ == "__main__":
    parser = Mm131Parser()
    # 使用并行下载
    parser.use_parallel = True # 这个网站我运行不起来，他许多参数都没有传进去，比如save方法中的路径是根据classMm131Parser传经去的方式来确定的
    # 是否保存
    # parser.save = True
